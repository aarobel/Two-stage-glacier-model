...
parameters.bedslope = -2*hg_orig/(beta*lambda*xg_orig);
...
%     parameters.bedslope = mts(j)*tss_stab_slow;
%     parameters.icedivide = b_orig-(parameters.bedslope*xg_orig);
%     hg_init = -(rho_w/rho_i)*Base(xg,parameters);
%     h = h_orig-(hg_orig-hg_init);
    parameters.sill_min = xg_orig;
    parameters.sill_max = xg_orig+20e3;
    parameters.sill_slope = tss_stab_slow*mts(j);
...
for t = 1:1e3 %100kyr to steady state
...
figure(3);set(3,'units','normalized','position',[0.1 0.1 0.7 0.45]);
subplot(1,2,1);
...
parameters.grid.n_nodes = 1000;      %Horizontal Resolution
parameters.grid.n2_nodes = 40;
parameters.grid.gz_nodes = 276;      %Horizontal Resolution in grounding zone
parameters.grid.sigma_gz = 0.97;
parameters.Dupont_G = 0;          %lateral shear stress
...
load ConvergedWeertman_linearbed5e-4_ac0.3_but40_N1000_276.mat   %load converged solution from earlier run
...
figure(3);set(3,'units','normalized','position',[0.1 0.1 0.7 0.45]);
subplot(1,2,1);
plot(time_all./1e3,(xg_all-xg_orig)./1e3,'b:','linewidth',4);hold on;
ylim([0 5])
xlim([0 10])
set(gca,'fontsize',24)
xlabel('Time (kyr)','fontsize',24)
...
% figure(3);
% subplot(1,2,1);
% plot(time_all./1e3,(xg_all-xg_orig)./1e3,'r--','linewidth',4);hold on;
% ylim([0 5])
% set(gca,'fontsize',24)
% xlabel('Time (kyr)','fontsize',24)
% ylabel('Grounding Line Deviation (km)','fontsize',24)
...
parameters.sill_min = xg_orig;
parameters.sill_max = xg_orig+20e3;
parameters.sill_slope = tss_stab_slow*0.7;
...
xlabel('Time (kyr)','fontsize',24)
ylabel('Grounding Line Deviation (km)','fontsize',24)
text(0.03,1.0,'a','Units', 'Normalized', 'VerticalAlignment', 'Top','fontsize',40)
...
xlabel('Time (yr)','fontsize',24)
ylabel('Grounding Line Deviation (km)','fontsize',24)
text(0.03,1.0,'b','Units', 'Normalized', 'VerticalAlignment', 'Top','fontsize',40)
...
figure(3)
subplot(1,2,1);
plot(time_all./1e3,(xg_all-xg_orig)./1e3,'k:','linewidth',4);hold on;
legend({'Two-Stage: $2 \bar{b}_x^{*S}$','Two-Stage: $0.7\bar{b}_x^{*S}$','Two-Stage: $\bar{b}_x^{*F}$','Flowline:   $2 \bar{b}_x^{*S}$','Flowline:   $0.7\bar{b}_x^{*S}$','Flowline:   $\bar{b}_x^{*F}$'},'Interpreter','LaTeX','Location','NorthEast')
subplot(1,2,2);
